  ...
  for(var j = 0; j < cars.length; j++)
  {
      if(cars[j].id == rental.car_id)
      {
        ...
        result +=  "#" + rental.id + ":<br>" + driver + " rented the "
                ...
                + "E</span><br>"
                + computeCommission(duration, price, deductibleReduction);
        if(deductibleReduction)
        {
          result += " (including a " + duration * 4
                    + "E deductible reduction fee [4E/day])";
        }
        result +=  "<br><br>";
        break;
      }
...
function computeCommission(duration, price, deductibleReduction)
{
  ...
  var result = "The commission (30%) is divided as following:<br>"
            + "&nbsp;- insurance : " + Math.round(10 * halfCom) / 10 + "E<br>"
            + "&nbsp;- assistance : " + duration + "E<br>"
            + "&nbsp;- drivy : " + Math.round(10 * drivy) / 10 + "E";
